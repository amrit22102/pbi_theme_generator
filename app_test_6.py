...
# ── PRESET CALLBACK — with st.rerun() for live preview update ────────────────
...
with left:
    ...
    with st.expander("🔤  Global Font Settings", expanded=True):
        ...
        gfont_changed = (new_gff != st.session_state.global_font_face or new_gfs != st.session_state.global_font_size or new_gfc != st.session_state.global_font_color)
        if gfont_changed:
    ...
    with st.expander("🎨  Data Colors", expanded=True):
        ...
        colors_changed = any(
            st.session_state.vis_custom.get(_vt, {}).get(f"color{i}") != [new_c0, new_c1, new_c2, new_c3][i]
            for _vt in sel_types_multi for i in range(4)
        )
        for _vt in sel_types_multi:
    ...
    with st.expander("🔤  Font", expanded=True):
        ...
        font_changed = any(
            st.session_state.vis_custom.get(_vt, {}).get("fontFace") != new_ff or
            st.session_state.vis_custom.get(_vt, {}).get("fontSize") != new_fs or
            st.session_state.vis_custom.get(_vt, {}).get("fontColor") != new_fc
            for _vt in sel_types_multi
        )
        for _vt in sel_types_multi:
    ...
    if x_app:
        ...
        with st.expander("↔  X-Axis"):
            ...
            xaxis_changed = any(
                st.session_state.vis_custom.get(_vt, {}).get(f) != v
                for _vt in x_app for f, v in [
                    ("xAxisShow", new_xas), ("xAxisTitle", new_xat), ("xAxisTitleText", new_xatt),
                    ("xAxisGridStyle", new_xags), ("xAxisGridColor", new_xagc)
                ]
            )
            for _vt in x_app:
    ...
    if y_app:
        ...
        with st.expander("↕  Y-Axis"):
            ...
            yaxis_changed = any(
                st.session_state.vis_custom.get(_vt, {}).get(f) != v
                for _vt in y_app for f, v in [
                    ("yAxisShow", new_yas), ("yAxisTitle", new_yat), ("yAxisTitleText", new_yatt),
                    ("yAxisGridStyle", new_yags), ("yAxisGridColor", new_yagc)
                ]
            )
            for _vt in y_app:
    ...
    if leg_app:
        ...
        with st.expander("📌  Legend"):
            ...
            legend_changed = any(
                st.session_state.vis_custom.get(_vt, {}).get(f) != v
                for _vt in leg_app for f, v in [
                    ("legendShow", new_ls), ("legendPosition", new_lp), ("legendFontSize", new_lfs)
                ]
            )
            for _vt in leg_app:
