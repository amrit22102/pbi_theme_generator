...
with left:
    ...
    if "vis_multidd" not in st.session_state:
        st.session_state["vis_multidd"] = ["Column Chart"]
    ...
    # Ensure all selected visuals have their custom state initialised
    for _vt in sel_types_multi:
        get_vis_custom(_vt)

    # Drive the shared controls from the FIRST selected visual's current values
    ...
    # ── 1. DATA COLORS — always shown, writes to all selected visuals ─────────
    with st.expander("🎨  Data Colors", expanded=True):
        ca, cb = st.columns(2)
        with ca:
            new_c0 = st.color_picker("Series 1", p["color0"], key=f"c0_{sel_type}")
            new_c2 = st.color_picker("Series 3", p["color2"], key=f"c2_{sel_type}")
        with cb:
            new_c1 = st.color_picker("Series 2", p["color1"], key=f"c1_{sel_type}")
            new_c3 = st.color_picker("Series 4", p["color3"], key=f"c3_{sel_type}")
        for _vt in sel_types_multi:
            st.session_state.vis_custom[_vt]["color0"] = new_c0
            st.session_state.vis_custom[_vt]["color1"] = new_c1
            st.session_state.vis_custom[_vt]["color2"] = new_c2
            st.session_state.vis_custom[_vt]["color3"] = new_c3

    # ── 2. FONT — always shown, writes to all selected visuals ────────────────
    with st.expander("🔤  Font", expanded=True):
        new_ff = st.selectbox("Family", FONT_OPTIONS, index=FONT_OPTIONS.index(p["fontFace"]) if p["fontFace"] in FONT_OPTIONS else 0, key=f"ff_{sel_type}")
        new_fs = st.slider("Size", 7, 20, p["fontSize"], key=f"fs_{sel_type}")
        new_fc = st.color_picker("Color", p["fontColor"], key=f"fc_{sel_type}")
        for _vt in sel_types_multi:
            st.session_state.vis_custom[_vt]["fontFace"]  = new_ff
            st.session_state.vis_custom[_vt]["fontSize"]  = new_fs
            st.session_state.vis_custom[_vt]["fontColor"] = new_fc

    # ── 3. X-AXIS — shown when at least one selected visual supports it ───────
    x_applicable = [vt for vt in sel_types_multi if vt not in no_x]
    if x_applicable:
        ref_xp = get_vis_custom(x_applicable[0])
        with st.expander("↔  X-Axis"):
            new_xas  = st.toggle("Show Axis",  ref_xp["xAxisShow"],  key=f"xas_{sel_type}")
            new_xat  = st.toggle("Show Title", ref_xp["xAxisTitle"], key=f"xat_{sel_type}")
            new_xatt = st.text_input("Title Text", ref_xp["xAxisTitleText"], key=f"xatt_{sel_type}")
            new_xags = st.selectbox("Grid Style", GRIDLINE_STYLES, index=GRIDLINE_STYLES.index(ref_xp["xAxisGridStyle"]), key=f"xags_{sel_type}")
            new_xagc = st.color_picker("Grid Color", ref_xp["xAxisGridColor"], key=f"xagc_{sel_type}")
            for _vt in x_applicable:
                st.session_state.vis_custom[_vt]["xAxisShow"]      = new_xas
                st.session_state.vis_custom[_vt]["xAxisTitle"]     = new_xat
                st.session_state.vis_custom[_vt]["xAxisTitleText"] = new_xatt
                st.session_state.vis_custom[_vt]["xAxisGridStyle"] = new_xags
                st.session_state.vis_custom[_vt]["xAxisGridColor"] = new_xagc

    # ── 4. Y-AXIS — shown when at least one selected visual supports it ───────
    y_applicable = [vt for vt in sel_types_multi if vt not in no_y]
    if y_applicable:
        ref_yp = get_vis_custom(y_applicable[0])
        with st.expander("↕  Y-Axis"):
            new_yas  = st.toggle("Show Axis",  ref_yp["yAxisShow"],  key=f"yas_{sel_type}")
            new_yat  = st.toggle("Show Title", ref_yp["yAxisTitle"], key=f"yat_{sel_type}")
            new_yatt = st.text_input("Title Text", ref_yp["yAxisTitleText"], key=f"yatt_{sel_type}")
            new_yags = st.selectbox("Grid Style", GRIDLINE_STYLES, index=GRIDLINE_STYLES.index(ref_yp["yAxisGridStyle"]), key=f"yags_{sel_type}")
            new_yagc = st.color_picker("Grid Color", ref_yp["yAxisGridColor"], key=f"yagc_{sel_type}")
            for _vt in y_applicable:
                st.session_state.vis_custom[_vt]["yAxisShow"]      = new_yas
                st.session_state.vis_custom[_vt]["yAxisTitle"]     = new_yat
                st.session_state.vis_custom[_vt]["yAxisTitleText"] = new_yatt
                st.session_state.vis_custom[_vt]["yAxisGridStyle"] = new_yags
                st.session_state.vis_custom[_vt]["yAxisGridColor"] = new_yagc

    # ── 5. LEGEND — shown when at least one selected visual supports it ───────
    leg_applicable = [vt for vt in sel_types_multi if vt not in no_leg]
    if leg_applicable:
        ref_lp = get_vis_custom(leg_applicable[0])
        with st.expander("📌  Legend"):
            new_ls  = st.toggle("Show Legend", ref_lp["legendShow"], key=f"ls_{sel_type}")
            new_lp  = st.selectbox("Position", LEGEND_POSITIONS, index=LEGEND_POSITIONS.index(ref_lp["legendPosition"]) if ref_lp["legendPosition"] in LEGEND_POSITIONS else 0, key=f"lp_{sel_type}")
            new_lfs = st.slider("Font Size", 7, 16, ref_lp["legendFontSize"], key=f"lfs_{sel_type}")
            for _vt in leg_applicable:
                st.session_state.vis_custom[_vt]["legendShow"]     = new_ls
                st.session_state.vis_custom[_vt]["legendPosition"] = new_lp
                st.session_state.vis_custom[_vt]["legendFontSize"] = new_lfs
...
with right:
    ...
    # JSON snippet — global font block + ALL selected visuals' blocks
    export_data = build_export()
    global_snippet = {
    ...
    vis_snippet = {
        vt: export_data["visualStyles"].get(vt, BASE_THEME["visualStyles"].get(vt, {}))
        for vt in sel_types_multi
    }
...
<body>
  ...
  <div class="section-label">Theme JSON — Global Font + All Selected Visual Blocks</div>
