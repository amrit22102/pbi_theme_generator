...
def get_vis_custom(vtype):
    if vtype not in st.session_state.vis_custom:
        ...
        ff = preset.get("fontFace", "Segoe UI")
        fc = preset.get("fontColor", t.get("foreground","#252423"))
        ...
        st.session_state.vis_custom[vtype] = {
            ...
            "fontFace": ff, "fontSize": 10, "fontColor": fc,
...
def render_column(p, w=600, h=330, label="Column Chart"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,p["fontColor"],w)}{_hgrids(p,pl,pb,w,h)}{bars}{_axes(p,w,h,pl,pb)}{_legend(p,w,h,[p["color0"],p["color1"],p["color2"]])}</svg>'
...
def render_bar(p, w=600, h=330, label="Bar Chart"):
    ...
    pb=16
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,p["fontColor"],w)}{vg}{bars}<line x1="{pl}" y1="26" x2="{pl}" y2="{h-pb}" stroke="{ax}" stroke-width="1"/>{_legend(p,w,h,[p["color0"],p["color1"],p["color2"]])}</svg>'
...
def render_line(p, w=600, h=330, label="Line Chart", filled=False):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,p["fontColor"],w)}{_hgrids(p,pl,pb,w,h)}{paths}{xlbls}{_axes(p,w,h,pl,pb)}{_legend(p,w,h,[p["color0"],p["color1"],p["color2"]])}</svg>'
...
def render_pie(p, w=600, h=330, donut=False, label="Pie Chart"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{"".join(paths)}{center}{leg}{_title(label,fg,w)}</svg>'
...
def render_scatter(p, w=600, h=330, label="Scatter Plot"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,p["fontColor"],w)}{_hgrids(p,pl,pb,w,h)}{dots}{_axes(p,w,h,pl,pb)}{_legend(p,w,h,[p["color0"],p["color1"],p["color2"]])}</svg>'
...
def render_card(p, w=600, h=330, label="Card Visual"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,fg,w)}{tiles}</svg>'
...
def render_matrix(p, w=600, h=330, label="Matrix / Table"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,fg,w)}{hdr}{data}{sep}</svg>'
...
def render_slicer(p, w=600, h=220, label="Slicer"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,fg,w)}{chips}</svg>'
...
def render_waterfall(p, w=600, h=330, label="Waterfall Chart"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,fg,w)}{_hgrids(p,pl,pb,w,h)}{bars}<line x1="{pl}" y1="{h-pb}" x2="{w-8}" y2="{h-pb}" stroke="{p["xAxisGridColor"]}" stroke-width="1"/></svg>'
...
def render_funnel(p, w=600, h=330, label="Funnel Chart"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,fg,w)}{bars}</svg>'
...
def render_gauge(p, w=600, h=330, label="Gauge"):
    ...
    return f'<svg width="{w}" height="{h}" xmlns="http://www.w3.org/2000/svg"><rect width="{w}" height="{h}" fill="{bg}" rx="6"/>{_title(label,fg,w)}{track}{fill}{needle}<text x="{cx}" y="{cy-int(r*0.25)}" text-anchor="middle" font-size="36" font-weight="700" fill="{p["color0"]}" font-family="Segoe UI,sans-serif">72%</text><text x="{cx}" y="{cy-int(r*0.05)}" text-anchor="middle" font-size="10" fill="{fg}" opacity=".4" font-family="Segoe UI,sans-serif">of target 85%</text></svg>'
...
with left:
    ...
    st.markdown('<p style="font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:#2a3555;margin:14px 0 5px">Select Visual</p>', unsafe_allow_html=True)
    sel_label = st.selectbox("_vs", VISUAL_LABELS,
        index=VISUAL_LABELS.index(TYPE_TO_LABEL.get(st.session_state.selected_type, "Column Chart")),
        label_visibility="collapsed", key="vis_dd")
    sel_type = VISUAL_TYPES[VISUAL_LABELS.index(sel_label)]
...
with right:
    ...
    svg      = render_visual(sel_type, p, w=660, h=360)
    snippet  = {sel_type: export_data["visualStyles"].get(sel_type, BASE_THEME["visualStyles"].get(sel_type, {}))}
    json_txt = json.dumps(snippet, indent=2)
...
<body>
  <div class="section-label">Active Preset</div>
  ...
  <div class="section-label">Live Preview — {sel_label}</div>
  <div class="vis-card">{svg}</div>

  <div class="section-label">Theme JSON — Active Visual Block</div>
